...
int birthdayCakeCandles(vector<int> candles) {
    unordered_map<int, int> heightToCountMap;
    int max = candles[0];
    for(auto elem : candles) {
        if(heightToCountMap.find(elem) == heightToCountMap.end())
            heightToCountMap.insert(make_pair(elem, 1));
        else {
            heightToCountMap[elem]++;
        }
        if(max < elem)
            max = elem;
    }
    return heightToCountMap[max];
}
